## Description
 ...
 This process will review all elements in selected dimensions (you can specify a single dimension,  multiple dimensions or wildcards to match dimensions) and hierarchies and will generate a `csv`  file listing all elements with unusual characters.  
 Control dimensions are ignored.  
     
**Note:**  
    
 - `pDim`: Specify which dimensions to validate. When specifying a dimension name, wildcards can    be specified by using the `*` and `?` characters. A list of dimensions can also be entered with    a delimiter (e.g. `v*&plan*` will process all dimensions starting with `v` and `plan`). If  
